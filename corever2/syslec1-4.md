...
## axiosの復習
### 非同期関数について
非同期関数は関数の処理がおわる前に次の処理に行く関数でした。

`fs.readFile`を使ってみましょう。
``` javascript
fs.readFile("./text.txt", function(data,error) {
    if (error) {
...
以上を実行すると、`fs.readFile`の実行が終わる前に、helloとコンソールに表示されますね。

### axiosについて
axiosを使いましょう。
npmを使ってaxiosをいれて、そのあと、適当なウェブページに対して、GET通信をしてみましょう！
